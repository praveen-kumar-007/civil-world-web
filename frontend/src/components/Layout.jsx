...
export default function Layout({ children }) {
  ...
  const socialLinks =
    Array.isArray(content?.data?.socialLinks) &&
    content.data.socialLinks.length > 0
      ? content.data.socialLinks
      : defaultContent.data.socialLinks;
  const playStoreLink =
    content?.data?.playStoreLink || defaultContent.data.playStoreLink;
  const brandName = content?.meta?.brandName || defaultContent.meta.brandName;
  const brandTagline =
    content?.meta?.brandTagline || defaultContent.meta.brandTagline;
  const footerText =
    content?.footer?.description || defaultContent.footer.description;
  const hsbtePortal =
      ...
      <header className="site-header">
        <nav className="nav container" aria-label="Primary navigation">
          <NavLink to="/" className="brand" onClick={() => setMenuOpen(false)}>
            <img className="brand-logo" src="/logo.png" alt="Civil World logo" />
            <span>
              <span className="brand-text">{brandName}</span>
              <span className="brand-subtext">{brandTagline}</span>
            </span>
          </NavLink>
      ...
      <footer className="site-footer">
        <div className="container footer-grid">
          ...
          <div className="footer-col-card">
            ...
            <a
              href={hsbtePortal?.url || "https://hsbte.org.in/"}
              target="_blank"
              rel="noopener noreferrer"
              className="hsbte-footer-card"
            >
              <img
                className="hsbte-footer-logo"
                src={
                  hsbtePortal?.logo ||
              ...
              />
              <span>
                <strong>{hsbtePortal?.title || "HSBTE Result Portal"}</strong>
                <small>{hsbtePortal?.ctaLabel || "Check Results"}</small>
              </span>
            </a>
          </div>
        </div>
